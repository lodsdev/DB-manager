...
local mtasa = _G
local DBManager = __TS__Class()
DBManager.name = "DBManager"
function DBManager.prototype.____constructor(self, url, dbName)
    self.dbName = dbName
    self.dbConnection = dbConnect("sqlite", ("" .. url .. "/" .. dbName) .. ".db")
    if not self.dbConnection then
        return error(''..getResourceName(getThisResource())..': Failed to connect to database '..dbName..'!')
    end
    outputDebugString("Database connected sucessfully!")
end
function DBManager.prototype.getDB(self)
    return self.dbConnection
end
_G.DBManager = DBManager

local InvTable = __TS__Class()
InvTable.name = "InvTable"
function InvTable.prototype.____constructor(self, dbManager, tblName, tblDefinitions)
    self.dbManager = dbManager
    self.tblDefinitions = tblDefinitions
    self.tblName = tblName
end
function InvTable.prototype.create(self)
    dbExec(
        self.dbManager:getDB(),
        "CREATE TABLE IF NOT EXISTS " .. self.tblName .. " (" ..self.tblDefinitions .. ")"
    )
end
function InvTable.prototype.delete(self)
    dbExec(
        self.dbManager:getDB(),
        "DROP TABLE " .. self.tblName
    )
end
function InvTable.prototype.getTblName(self)
    return self.tblName
end
_G.InvTable = InvTable


local SQLRepo = __TS__Class()
SQLRepo.name = "SQLRepo"
function SQLRepo.prototype.____constructor(self, dbManager, InvTable, dto)
    self.dbManager = dbManager
    self.InvTable = InvTable
    self.dto = dto
end
function SQLRepo.prototype.create(self, dto)
    ...
    dbExec(
        self.dbManager:getDB(),
        'INSERT INTO '..self.InvTable..' VALUES ('..self.dto..')'
    )
end
function SQLRepo.prototype.delete(self, id)
    dbExec(
        self.dbManager:getDB(),
        "DELETE FROM " .. self.InvTable .. " WHERE " .. tostring(id) .. " = " .. tostring(id)
    )
end
function SQLRepo.prototype.update(self, id, dto)
    dbExec(
        self.dbManager:getDB(),
        'UPDATE ' .. self.tblName .. ' SET ' .. tostring(id) .. ' = ? WHERE ' .. tostring(id) .. ' = ' .. tostring(id), dto
    )
end
function SQLRepo.prototype.findAll(self)
    return __TS__AsyncAwaiter(function()
        return nil, __TS__New(
            __TS__Promise,
            function(____, resolve, reject)
                dbQuery(
                    function(qh)
                    ...
                    {},
                    self.dbManager:getDB(),
                    "SELECT * FROM " .. self.InvTable
                )
            end
        )
    end)
end
function SQLRepo.prototype.findOne(self, id)
    return __TS__AsyncAwaiter(function()
        return nil, __TS__New(
            __TS__Promise,
            function(____, resolve, reject)
                dbQuery(
                    ...
                    {},
                    self.dbManager:getDB(),
                    'SELECT * FROM ' .. self.tblName .. ' WHERE ' .. tostring(id) .. ' = ' .. tostring(id)
                )
            end
        )
    end)
...
local TableInvRepo = __TS__Class()
TableInvRepo.name = "TableInvRepo"
function TableInvRepo.prototype.____constructor(self, sqlRepo)
    self.sqlRepo = sqlRepo
    self:init()
end
function TableInvRepo.prototype.create(self, dto)
    self.datas:push(dto)
end
function TableInvRepo.prototype.delete(self, id)
    self.datas:set(id, nil)
end
function TableInvRepo.prototype.update(self, id, value)
    self.datas:set(id, value)
end
function TableInvRepo.prototype.findAll(self)
    return self.datas
end
function TableInvRepo.prototype.findOne(self, id)
    return self.datas:get(getIndexFromId(self.datas, id))
end
function TableInvRepo.prototype.init(self)
    return __TS__New(
        __TS__Promise,
        function(____, resolve, reject)
...
_G.TableInvRepo = TableInvRepo



local RepoService = __TS__Class()
RepoService.name = "RepoService"
function RepoService.prototype.____constructor(self, sqlRepo, tableRepo)
    self.sqlRepo = sqlRepo
    self.tableRepo = tableRepo
end
function RepoService.getInstance(self, sqlRepo, tableRepo)
    if not RepoService.instance then
        RepoService.instance = __TS__New(RepoService, sqlRepo, tableRepo)
    end
    return RepoService.instance
end
function RepoService.prototype.create(self, dto)
    self.sqlRepo:create(dto)
    self.tableRepo:create(dto)
end
function RepoService.prototype.delete(self, id)
    self.sqlRepo:delete(id)
    self.tableRepo:delete(id)
end
function RepoService.prototype.update(self, id, data)
    self.sqlRepo:update(id, data)
    self.tableRepo:update(id, data)
end
function RepoService.prototype.findAll(self)
    return __TS__AsyncAwaiter(function()
        local invRepo = self.tableRepo:findAll()
        if not invRepo then
            invRepo = __TS__Await(
                ____catch,
...
function RepoService.prototype.findOne(self, id)
    return __TS__AsyncAwaiter(function()
        local invRepo = self.tableRepo:findOne(id)
        if not invRepo then
            invRepo = __TS__Await(
                ____catch,
                self.sqlRepo:findOne(id)
            )
        end
        return nil, invRepo
    end)
end
_G.RepoService = RepoService